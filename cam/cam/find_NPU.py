...
class BallDetectNode(Node):
    def __init__(self):
        ...
        self.model_path = "yolov5s.om"  # 请替换为实际模型路径
        self.device_id = 0
    ...
    def process_frame(self):
        ...
        # 预处理: resize, BGR2RGB, normalization, NHWC->NCHW
        ...
        # 推理
        outputs = self.session.infer([input_img])
        # 后处理: 假设outputs[0]为检测结果，格式需根据实际模型输出调整
        detections = outputs[0]  # [N, 6] [x1, y1, x2, y2, conf, class]
        for det in detections:
            ...
            class_name = str(int(cls_id))  # 没有names属性，直接显示类别id
