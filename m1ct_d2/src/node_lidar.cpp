...
std::shared_ptr<Serial_Port> output_serial;
    ...
    void topic_callback(const std_msgs::msg::UInt16::SharedPtr msg) const
    {
		switch (msg->data)
		{
			...
			case 6:
				...
				node_lidar.lidar_general_info.frequency_max = 30;
				node_lidar.lidar_general_info.frequency_min = 20;
				break;
			/*低转速*/
			case 7:
			...
			for (size_t pos = 0; pos < count; ++pos)
			{
				if (local_buf[pos].sync_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT)
				{
					if ((local_scan[0].sync_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT))
					{
						...
						/*频率异常超过30秒，触发异常状态*/
						if(local_scan[0].scan_frequence > node_lidar.lidar_general_info.frequency_max 
						    || local_scan[0].scan_frequence < node_lidar.lidar_general_info.frequency_min)
						{
...
void send_lidar_data(LaserScan &outscan)
{
	...
	{	
		...
		if(node_lidar.lidar_status.isConnected)
		{
			...
			float min_angle = 0.0;
			float ranges[4] = {0};
			float angle_diff[4] = {360, 360, 360, 360};
			const float target_angle[4] = {0.0, 90.0, 180.0, 270.0};
			for(auto& point : outscan.points){
				for(int i = 0; i < 4; i++){
					float diff = fabs(point.angle - target_angle[i]);
					if(diff > 180.0f)
						diff = 360.0f - diff;
					if(diff < angle_diff[i]){
						angle_diff[i] = diff;
						ranges[i] = point.range;
					}
				}
			}
			for(auto& point : outscan.points){
				if(point.range >= outscan.config.min_range && 
					point.range <= outscan.config.max_range){
			...
			int min_dist_cm = static_cast<int>(min_dist * 100.0f);
			int min_angle_int = (int)min_angle;
			//printf("min_angle: %d, range: %d\n", min_angle_int, min_dist_cm);
			// 准备串口输出数据
			uint8_t output_buffer[11];
			output_buffer[0] = 0xAA;
			output_buffer[1] = 0x55;

			uint8_t output_buffer2[7];
			output_buffer2[0] = 0xAA;
			output_buffer2[1] = 0x33;
			output_buffer2[2] = (min_angle_int >> 8) & 0xFF;
			output_buffer2[3] = min_angle_int & 0xFF;
			output_buffer2[4] = (min_dist_cm >> 8) & 0xFF;
			output_buffer2[5] = min_dist_cm & 0xFF;
			output_buffer2[6] = 0x5D;
			
			// 填充距离数据（单位：厘米）
			for (int i = 0; i < 4; i++) {
				uint16_t dist_cm = 0;
				if (angle_diff[i] < 5.0f) {
					dist_cm = static_cast<uint16_t>(ranges[i] * 100.0f);
				}
				
				output_buffer[2 + i*2] = (dist_cm >> 8) & 0xFF;        
				output_buffer[3 + i*2] = dist_cm & 0xFF; 
			}
			
			output_buffer[11 - 1] = 0x5D;
			
			// 通过串口输出
			if (output_serial) {
				output_serial->write_data(output_buffer, 11);
				output_serial->write_data(output_buffer2, 7);
			}
			// for(int i = 0; i < 4;i++){
			// 	if(angle_diff[i] <= 5.0f){
			// 		printf("angle %.1f = %.2f\n",target_angle[i], ranges[i]);
			// 	}
			// 	else{
			// 		printf("angle %.1f = N/A\n", target_angle[i]);
			// 	}
...
bool initialize()
{
	...
	// 初始化输出串口（用于发送角度数据）
    // const char* output_port = "/dev/ttyUSB1"; // 根据实际硬件连接修改
    // int output_baudrate = 115200;
    
    // output_serial = make_shared<Serial_Port>(output_port, output_baudrate, 
    //                                         Timeout::simpleTimeout(DEFAULT_TIMEOUT));
    // if (!output_serial->open()) {
    //     printf("Failed to open output serial port! Using console output only.\n");
    // } else {
    //     printf("Output serial port opened successfully.\n");
    // }
